...
public class BinarySearchTree<T extends Comparable<T>> extends AbstractSet<T> implements CheckableSortedSet<T> {
    ...
    private Node<T> remove(Node<T> node, T value) throws Exception {
        switch (value.compareTo(node.value)) {
            case 1:
                if (value.compareTo(node.value) != 0 && node.left == null && node.right == null) {
                    throw new Exception();
                }
            ...
            case -1:
                ...
                return node;
            default:
                return pictureSituation(node);
        }
    }

    private Node<T> pictureSituation(Node<T> node) {
        //1) Нет детей.
        if (node.right == null && node.left == null) {
            ...
            Node<T> min = new Node(getMin(temp).value);
    ...
    public class BinarySearchTreeIterator implements Iterator<T> {
        ...
        private void insertInLeftBranch(Node<T> n) { // складывает узлы начиная от корня в левую нижнюю сторону
            if (n != null) {
                iteratorStack.push(n);
                insertInLeftBranch(n.left);
            }
        }
        /**
        ...
        public T next() {
            ...
            currentNode = iteratorStack.pop();
            Node<T> outputNode = currentNode;

            insertInLeftBranch(outputNode.right);
